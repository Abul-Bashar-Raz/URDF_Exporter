...
def write_link_urdf(joints_dict, repo, links_xyz_dict, file_name, inertial_dict, material_dict):
    ...
    with open(file_name, mode='a') as f:
        ...
        # Map child link to joint xyz for links that are connected by a joint.
        child_joint_xyz = {}
        for joint_name in joints_dict:
            child_name = re.sub('[ :()]', '_', joints_dict[joint_name]['child'])
            if child_name in child_joint_xyz:
                ...
                % (child_name), "Error!")
                quit()
            child_joint_xyz[child_name] = joints_dict[joint_name]['xyz']

        # Export all non-base links so mesh/link counts stay consistent.
        for name in inertial_dict:
            if name == 'base_link':
                continue
            if name not in material_dict:
                continue

            xyz = child_joint_xyz.get(name, [0, 0, 0])
            center_of_mass = [i - j for i, j in zip(inertial_dict[name]['center_of_mass_float'], xyz)]
            link = Link.Link(name=name, xyz=xyz,
